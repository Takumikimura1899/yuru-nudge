...
/** タスク完了時、羊（NudgeySheep）を happy 表示にし続ける時間 */
...
export function useChat(init: { initialMessages: ChatMessageData[]; initialIntensity: Intensity }) {
  ...
  // タスク完了の喜び演出タイマーは unmount 時に必ず解除する
  useEffect(() => () => clearTimeout(celebrationTimeoutRef.current), []);

  /**
   * タスク完了（reaction:completed かつ成功）のたびに呼ぶ。羊を一定時間 happy 表示にする。
   * 連続完了時は最初のタイマーを打ち切って再セットする（debounce-reset。早期に false へ
   * 戻ってしまうのを防ぐ）。
   */
  function celebrate() {
  ...
  async function react(seedId: string, reaction: ReactionKind) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        // 完了タスクのみ喜ぶ（archived/softened では喜ばない）。alreadyReacted は新規完了ではない
        if (reaction === "completed" && !result.alreadyReacted) {
          celebrate();
        }
      } else {
