...
describe("fetchPouchSeeds（タネ袋一覧）", () => {
  test("status in ('pending','nudged') で絞り込み、nudged優先→created_at asc→id ascの順に並べる", async () => {
    const db = createMockDb();
    // 実装は eb.case().when("status","=","nudged").then(0).else(1).end() を
    // orderBy() のコールバック引数に渡している。Kysely の ExpressionBuilder を模した
    // フェイクをコールバックへ渡し、実際に呼び出させて構造を検証する（mutterings.test.ts の fakeEb パターン）
    const fakeEb = {
      case: vi.fn().mockReturnThis(),
      when: vi.fn().mockReturnThis(),
      // Kysely の CaseBuilder が実際に持つメソッド名を模している（Promise の thenable とは無関係）
      // eslint-disable-next-line unicorn/no-thenable
      then: vi.fn().mockReturnThis(),
      else: vi.fn().mockReturnThis(),
      end: vi.fn(() => "nudged-first"),
    };
    db.orderBy = vi.fn((arg: unknown) => {
    ...
    await fetchPouchSeeds(asDb(db), "test-user");

    expect(db.selectFrom).toHaveBeenCalledWith("seeds");
    expect(db.select).toHaveBeenCalledWith(["id", "processed_task", "status"]);
    expect(db.where).toHaveBeenCalledWith("user_id", "=", "test-user");
    expect(db.where).toHaveBeenCalledWith("status", "in", ["pending", "nudged"]);
    expect(fakeEb.case).toHaveBeenCalledTimes(1);
    expect(fakeEb.when).toHaveBeenCalledWith("status", "=", "nudged");
    expect(fakeEb.then).toHaveBeenCalledWith(0);
    expect(fakeEb.else).toHaveBeenCalledWith(1);
    expect(fakeEb.end).toHaveBeenCalledTimes(1);
    expect(db.orderBy).toHaveBeenCalledWith("created_at", "asc");
    expect(db.orderBy).toHaveBeenCalledWith("id", "asc");
  });

  test("取得行を seedId/task/status に整形して返す", async () => {
    const db = createMockDb();
    db.execute.mockResolvedValueOnce([
      { id: "seed-1", processed_task: "部屋を片付ける", status: "nudged" },
      { id: "seed-2", processed_task: "本を読む", status: "pending" },
    ]);

    const result = await fetchPouchSeeds(asDb(db), "test-user");

    expect(result).toEqual([
      { seedId: "seed-1", task: "部屋を片付ける", status: "nudged" },
      { seedId: "seed-2", task: "本を読む", status: "pending" },
    ]);
  });
});
